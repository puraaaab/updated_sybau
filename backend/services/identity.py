...
class GlobalIdentityManager:
    ...
    def get_or_create_face_identity(track_uuid: str, camera_id: str, face_embedding: list) -> str:
        ...
        try:
            # Query all existing global face identities in relational DB
            identities = db.query(GlobalIdentity).filter(GlobalIdentity.type == "person").all()
            
            best_match = None
            best_score = 0.85 # Cosine similarity threshold
            ...
            try:
                ...
                # Fallback to local in-memory scan if Qdrant is unavailable
            ...
            if best_match:
                ...
                if db_id:
                    ...
                    db.commit()
                    return db_id.identity_uuid

            # If no match, create a new POI identity
            import uuid
    ...
    def get_or_create_vehicle_identity(track_uuid: str, camera_id: str, reid_vector: list, license_plate: str = None) -> str:
        ...
        try:
            ...
            # Fallback to Re-ID vector matching in vector_db/Qdrant
            best_match = None
            best_score = 0.85
            ...
            # Create new visual vehicle identity
            import uuid
            short_uuid = str(uuid.uuid4())[:6].upper()
